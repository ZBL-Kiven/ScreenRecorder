...
class MicRecorder implements Encoder {
    ...
    private class RecordHandler extends Handler {
        ...
        private final LinkedList<MediaCodec.BufferInfo> mCachedInfo = new LinkedList<>();
        private final LinkedList<Integer> mOutputBufferIndices = new LinkedList<>();
        ...
        public void handleMessage(Message msg) {
            ...
            switch (msg.what) {
                ...
                case MSG_FEED_INPUT:
                    if (!mForceStop.get()) {
                        int index = pollInput();
                        if (VERBOSE) Log.d(TAG, "audio encoder returned input buffer index=" + index);
                        if (index >= 0) {
                ...
                case MSG_RELEASE_OUTPUT:
                    ...
                    mOutputBufferIndices.poll(); // Nobody care what it exactly is.
                    if (VERBOSE) Log.d(TAG, "audio encoder released output buffer index=" + msg.arg1 + ", remaining=" + mOutputBufferIndices.size());
                    pollInputIfNeed();
                    break;
                case MSG_STOP:
                    if (mMic != null) {
        ...
        private void offerOutput() {
            while (!mForceStop.get()) {
                MediaCodec.BufferInfo info = mCachedInfo.poll();
                if (info == null) {
                ...
                if (index < 0) {
                    info.set(0, 0, 0, 0);
                    mCachedInfo.offer(info);
                    break;
                }
                mOutputBufferIndices.offer(index);
        ...
        private void pollInputIfNeed() {
            if (mOutputBufferIndices.size() <= 1 && !mForceStop.get()) {
    ...
    private void feedAudioEncoder(int index) {
        ...
        if (!eos) {
            ...
            if (VERBOSE) Log.d(TAG, "Read frame data size " + read + " for index " + index + " buffer : " + offset + ", " + limit);
            if (read < 0) {
                read = 0;
            }
        }
        ...
        if (VERBOSE) Log.d(TAG, "Feed codec index=" + index + ", presentationTimeUs=" + pstTs + ", flags=" + flags);
    ...
    private long calculateFrameTimestamp(int totalBits) {
        ...
        if (VERBOSE) Log.i(TAG, "count samples pts: " + currentUs + ", time pts: " + timeUs + ", samples: " + samples);
    ...
    private static AudioRecord createAudioRecord(int sampleRateInHz, int channelConfig, int audioFormat) {
        ...
        @SuppressLint("MissingPermission") AudioRecord record = new AudioRecord(MediaRecorder.AudioSource.MIC, sampleRateInHz, channelConfig, audioFormat, minBytes * 2);
