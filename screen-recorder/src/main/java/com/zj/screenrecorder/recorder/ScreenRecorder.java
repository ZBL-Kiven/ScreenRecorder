...
class ScreenRecorder {
    ...
    private final LinkedList<MediaCodec.BufferInfo> mPendingAudioEncoderBuffers = new LinkedList<>();
    private final LinkedList<MediaCodec.BufferInfo> mPendingVideoEncoderBuffers = new LinkedList<>();
    ...
    private void muxVideo(int index, MediaCodec.BufferInfo buffer) {
        ...
        if (!mMuxerStarted || mVideoTrackIndex == INVALID_INDEX) {
            ...
            mPendingVideoEncoderBuffers.add(buffer);
            return;
        }
    ...
    private void muxAudio(int index, MediaCodec.BufferInfo buffer) {
        ...
        if (!mMuxerStarted || mAudioTrackIndex == INVALID_INDEX) {
            ...
            mPendingAudioEncoderBuffers.add(buffer);
            return;
        }
    ...
    private void startMuxerIfReady() {
        ...
        while ((info = mPendingVideoEncoderBuffers.poll()) != null) {
            Integer index = mPendingVideoEncoderBufferIndices.poll();
            if (index != null) muxVideo(index, info);
        }
        if (mAudioEncoder != null) {
            while ((info = mPendingAudioEncoderBuffers.poll()) != null) {
                Integer index = mPendingAudioEncoderBufferIndices.poll();
                if (index != null) muxAudio(index, info);
            }
        }
    ...
    private void prepareAudioEncoder() throws IOException {
        ...
        AudioEncoder.Callback callback = new AudioEncoder.Callback() {
            ...
            void onInputBufferAvailable(BaseEncoder encoder, int index) {
                super.onInputBufferAvailable(encoder, index);
            }
    ...
    private void stopEncoders() {
        ...
        mPendingAudioEncoderBuffers.clear();
        mPendingAudioEncoderBufferIndices.clear();
        mPendingVideoEncoderBuffers.clear();
